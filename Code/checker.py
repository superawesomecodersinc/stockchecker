...
if choice in yes:
   ...
   sys.stdout.write("Please try the script again, and respond with 'yes' or 'no'")
...
print ("")
choice = input("Would you like me to forecast the future of " + companynameupper + "/" + company + "'s share price? ").lower()
print ("")
if choice in yes:
    warnings.filterwarnings("ignore")
    numberofdays = int(input("How many days into the future would you like to predict? "))
    df = quandl.get("WIKI/" + companynameupper)
    df = df[['Adj. Close']]

    # Forecast variables
    forecast_out = int(numberofdays) # predicting 30 days into future
    df['Prediction'] = df[['Adj. Close']].shift(-forecast_out) #  label column with data shifted
    ...
    print ("")
    choice = input("Would you like the raw data before you see the graph? ").lower()
    if choice in yes:
        ...
        print("")
        plt.plot(forecast_prediction)
        plt.ylabel('Prediction for ' + companynameupper + "/" + company + "'s stock price")
        plt.show()
    else:
       sys.stdout.write("Please start the script again, and respond with 'yes' or 'no'")
elif choice in no:
    print("")
    print ("Okay, goodbye!")
else:
